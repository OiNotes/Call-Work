import { getShopByOwner, createShop, generateBitcoinAddress, verifyPayment } from '../utils/api.js';
import {
  sellerMenuKeyboard,
...
export async function handlePaymentHashInput(ctx) {
  try {
    ...
    await ctx.reply(`⏳ Проверяем платеж...\n\n` + `Это может занять несколько секунд.`);
...
export async function handleShopNameInput(ctx) {
  try {
    ...
    if (!validName) {
      await ctx.reply(
        '❌ Название содержит недопустимые символы. Используйте только буквы, цифры и пробелы.'
      );
      return;
    }
