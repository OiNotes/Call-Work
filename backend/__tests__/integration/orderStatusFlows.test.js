...
describe('Order status flows', () => {
  ...
  describe('GET /api/orders?shop_id', () => {
    it('returns only confirmed orders when filtered as active', async () => {
      const response = await authGet(`/api/orders?shop_id=${shop.id}&status=confirmed`)
        .expect(200);
    ...
    it('understands delivered/completed aliases', async () => {
      const response = await authGet(`/api/orders?shop_id=${shop.id}&status=delivered,completed`)
        .expect(200);
  ...
  describe('GET /api/orders/active/count', () => {
    it('counts confirmed orders only', async () => {
      const response = await authGet(`/api/orders/active/count?shop_id=${shop.id}`)
        .expect(200);
